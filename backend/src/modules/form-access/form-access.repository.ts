import { Pool } from 'pg'
import { role } from './form-access.types';

export class FormAccessRepository {
    constructor(private pool: Pool) { }

    async addAccess(formId: string, userId: string, role: role) {
    ...
    async removeAccess(formId: string, userId: string) {
        const { rows } = await this.pool.query("DELETE FROM form_access WHERE form_id = $1 AND user_id = $2", [formId, userId])
        return rows[0]
    }
